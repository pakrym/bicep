...
namespace Bicep.Core.UnitTests.Assertions
{
    ...
    public class ErrorBuilderAssertions : ReferenceTypeAssertions<DiagnosticBuilder.ErrorBuilderDelegate, ErrorBuilderAssertions>
    {
        ...
        public AndConstraint<ErrorBuilderAssertions> HaveCodeAndSeverity(string code, DiagnosticLevel level, string because = "", params object[] becauseArgs)
        {
            ErrorDiagnostic error = GetErrorFromSubject();

            using (new AssertionScope())
            {
                error.Should().HaveCodeAndSeverity(code, level, because, becauseArgs);
            }
